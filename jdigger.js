...
function fullscreen() {
	...
	if (i.requestFullscreen) {
		i.requestFullscreen(); fullscreen_flag = true;
	} else if (i.webkitRequestFullscreen) {
		i.webkitRequestFullscreen(); fullscreen_flag = true;
	} else if (i.mozRequestFullScreen) {
		i.mozRequestFullScreen(); fullscreen_flag = true;
	} else if (i.msRequestFullscreen) {
		i.msRequestFullscreen(); fullscreen_flag = true;
	}
}
...
function init_room(level) {
	...
	if (state != 'menu' && state != 'highscore' && state != 'input') {
        ...
        //setze Digger an seine Position
...
function draw_digger_death() {
	if (score_ges>1) idx[d_idx - 21] = 3.1; else idx[d_idx - 21] = 0.1;
	if (score_ges>0) idx[d_idx - 20] = 3.1; else idx[d_idx - 20] = 0.1;
	if (score_ges>2) idx[d_idx - 19] = 3.1; else idx[d_idx - 19] = 0.1;
	if (score_ges>3) idx[d_idx - 1] = 3.1; else idx[d_idx - 1] = 0.1;
	idx[d_idx] = 63.1;
	if (score_ges>4) idx[d_idx + 1] = 3.1; else idx[d_idx + 1] = 0.1;
	if (score_ges>6) idx[d_idx + 19] = 3.1; else idx[d_idx + 19] = 0.1;
	if (score_ges>5) idx[d_idx + 20] = 3.1; else idx[d_idx + 20] = 0.1;
	if (score_ges>7) idx[d_idx + 21] = 3.1; else idx[d_idx + 21] = 0.1;
	digger_die = false;
	digger_is_dead = true;
	ton_diamant = true;
}
    ...
    if (state == 'look' || state == 'init' || state == 'play') {
			...
			if ((state == 'play') && !digger_death && !digger_idle) {
				...
				if (digger_go == 'LEFT') {
					...
					// ? Ausgang
					else if (((idx[pre_l] == 41) || (idx[pre_l] == 42)) && !digger_half_step) {
						score_punkte += 100;
						next_raum = true;
					}
					// ? Geist
					else if ( ((idx[pre_l] >= 43) && (idx[pre_l] < 63)) && !digger_half_step) {
                        digger_death = true;
                        digger_die = true;
                    }
					// ? Stein
					else if ((idx[pre_l] == 7) && !digger_half_step) {
				...
				// ? HOCH
					...
					// ? Ausgang
					else if (((idx[pre_h] == 41) || (idx[pre_h] == 42)) && !digger_half_step) {
						score_punkte += 100;
						next_raum = true;
					}
					// ? Geist
					else if ( ((idx[pre_h] >= 43) && (idx[pre_h] < 63)) && !digger_half_step) {
                        digger_death = true;
                        digger_die = true;
                    }
					// ? Sand, Diamant oder Leer
				...
				// ? RECHTS
					...
					// ? Ausgang
					else if (((idx[pre_r] == 41) || (idx[pre_r] == 42)) && !digger_half_step) {
						score_punkte += 100;
						next_raum = true;
					}
					// ? Geist
					else if ( ((idx[pre_r] >= 43) && (idx[pre_r] < 63)) && !digger_half_step) {
                        digger_death = true;
                        digger_die = true;
                    }
					// ? Stein
					else if ((idx[pre_r] == 7) && !digger_half_step) {
						// ? Platz zum wegschieben
						if (idx[pre_rr] == 1) {
							// ! 2 Takte lang druecken
							if (stone_r) {
				...
				// ? RUNTER
                    ...
                    var pre_d = pre_idx + 20;
					// ? Diamant
					if ((idx[pre_d] == 3) && !digger_half_step) {
						score_ges++;
						score_punkte += 3;
						ton_diamant = true;
					}
					// ? Ausgang
					else if (((idx[pre_d] == 41) || (idx[pre_d] == 42)) && !digger_half_step) {
						score_punkte += 100;
						next_raum = true;
					}
					// ? Geist
					else if ( ((idx[pre_d] >= 43) && (idx[pre_d] < 63)) && !digger_half_step) {
                        digger_death = true;
                        digger_die = true;
                    }
					// ? Sand, Diamant oder Leer
					if ((idx[pre_d] < 4) && !digger_half_step) {
						idx[pre_idx] = 1.1;
						d_idx += 20;
						ton_schritt = true;
					}
			...
			//DIGGER TOETEN
			else if (digger_death && digger_die && !digger_is_dead) {
				draw_digger_death();
				digger_go = 'NONE';
			}

			//SPIELFELD AKTIVITAETEN
			if ((state == 'play') && !digger_half_step) {
				...
				for (var l = 1; l < 281; l++) {
					...
					//sichtbaren EXIT (41, 42) blinken lassen
					// 41 = exit, exit.gif
					// 42 = exit, wall.gif
			...
			//Diamant Blinksequenz (64 bis (max_diamond_blink-1))
			diamond_blink += 2;
...
setTimeout(draw_frame, 55);

}

function initDigger() {
	// Raum(n) initialisieren
	init_room(0);
    
	// Engine starten
	// - Berechnungen im Spielfeld
	// - Spielfeld malen
	draw_frame();
	
    // starte Tastatur-/Mausabfrage
	init_events();
}
